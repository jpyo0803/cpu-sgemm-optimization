...
py::array_t<float> py_matmul_tiling_2d(py::array_t<float> A, py::array_t<float> B) {
  auto bufA = A.request();
  auto bufB = B.request();
  ...
  constexpr int K = 1024;
  constexpr int N = 1024;
  constexpr int TileSize = 4; // 타일 크기
  ...
  // 로우레벨 C++ 연산 호출
  MatmulImplTiling2D<M, N, K, TileSize>(ptrA, ptrB, ptrResult);

  return result;
}

py::array_t<float> py_matmul_row_col_parallel_tiling_1d(py::array_t<float> A, py::array_t<float> B) {
  auto bufA = A.request();
  auto bufB = B.request();
  ...
  // 로우레벨 C++ 연산 호출
  MatmulImplRowColParallelTiling1D<M, N, K, TileSize>(ptrA, ptrB, ptrResult);
...
PYBIND11_MODULE(custom_backend, m) {
  ...
  m.def("matmul_tiling_2d", &py_matmul_tiling_2d,
